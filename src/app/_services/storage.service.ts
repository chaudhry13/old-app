...
import { Attachment } from "../_models/file";
...
export class StorageService extends GenericService {
  ...
  async listRiskAssessment(id: string): Promise<Attachment[]> {
    return this.http.get<Attachment[]>(this.apiBase + "/risk-assessment?riskAssessmentId=" + id).toPromise();
  }

  async listTemplate(id: string): Promise<Attachment[]> {
    return this.http.get<Attachment[]>(this.apiBase + "/template?templateId=" + id).toPromise();
  }

  async listTreatment(riskAssessmentId: string, treatmentId: string): Promise<Attachment[]> {
    return this.http.get<Attachment[]>(this.apiBase + "/treatment?riskAssessmentId=" + riskAssessmentId + "&treatmentId=" + treatmentId).toPromise();
  }

  async listIncidentReport(id: string): Promise<Attachment[]> {
    return this.http.get<Attachment[]>(this.apiBase + "/incident-report?incidentReportId=" + id).toPromise();
  }

  async listControl(id: string): Promise<Attachment[]> {
    return this.http.get<Attachment[]>(this.apiBase + "/control?controlId=" + id).toPromise();
  }

  async listAudit(controlId: string, auditId: string): Promise<Attachment[]> {
    return this.http.get<Attachment[]>(this.apiBase + "/audit?controlId=" + controlId + "&auditId=" + auditId).toPromise();
  }

  /// Delete endpoints
  async deleteRiskAssessment(id: string, fileName: string): Promise<any> {
    return this.http.delete<any>(this.apiBase + "/risk-assessment?riskAssessmentId=" + id + "&fileName=" + fileName).toPromise();
  }

  async deleteIncidentReport(id: string, fileName: string): Promise<any> {
    return this.http.delete<any>(this.apiBase + "/incident-report?incidentReportId=" + id + "&fileName=" + fileName).toPromise();
  }

  async deleteTemplate(id: string, fileName: string): Promise<any> {
    return this.http.delete<any>(this.apiBase + "/template?templateId=" + id + "&fileName=" + fileName).toPromise();
  }

  async deleteTreatment(id: string, treatmentId: string, fileName: string): Promise<any> {
    return this.http
      .delete<any>(this.apiBase + "/treatment?riskAssessmentId=" + id + "&treatmentId=" + treatmentId + "&fileName=" + fileName)
      .toPromise();
  }

  async deleteControl(id: string, fileName: string): Promise<any> {
    return this.http.delete<any>(this.apiBase + "/control?controlId=" + id + "&fileName=" + fileName).toPromise();
  }
