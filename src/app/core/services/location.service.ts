...
export class LocationService {
  ...
  public async getLocationFromLongLat(latitude: number, longitude: number): Promise<LocationViewModel> {
    ...
    return new Promise<LocationViewModel>((resolve, reject) => {
      this.geolocation
        ...
        .then((position) => {
          ...
          let location = this.getLocationFromLongLat(position.coords.latitude, position.coords.longitude);
          resolve(location);
        })
        .catch((error) => {
          this.toastService.show("We couldn't find any locations based on your position");
  ...
  public async populateFormWithLocation(form: FormGroup, location: LocationViewModel): Promise<void> {
    form.controls.address.setValue(location.address);
    form.controls.city.setValue(location.city);
    form.controls.countryId.setValue(await this.getCountryIdFromCode(location.country));
