...
export class ValidationService {
  ...
  numberValidation(
    ...
    if (this.checkNumberRange(answerForm, question)) {
      ...
      return {
        ...
        regExp: null,
        errorMsg:
          "Your response must be between " +
          question.numberOptions.from +
          " and " +
  ...
  private findRegularExpressionAndErrorMessage(
    question: Question
  ): QuestionValidation {
    ...
    switch (question.textOptions.type) {
      case QuestionTextType["Custom Regex"]:
        return {
          ...
          errorMsg:
            "The text should comply with the following regular expression: " +
            question.textOptions.regex.toString(),
        };
  ...
  private checkNumberRange(answerForm: FormGroup, question: Question) {
    return (
      +answerForm.controls.numberAnswer.value >= question.numberOptions.from &&
